...
export type SeriesPoint = { date: string; gross: number; net: number; orders: number };
export type Overview = { kpis: { grossSales: number; discounts: number; netSales: number; transactions: number; averageOrderValue: number; itemsSold: number; activeCustomers: number; lowStockCount: number }; salesSeries: SeriesPoint[]; payments: { name: string; value: number; color: string }[]; topProducts: { name: string; sold: number; revenue: number }[]; lowStock: { name: string; quantity: number; threshold: number; status: 'Low' | 'Out of stock' }[]; recentActivity: { type: string; title: string; time: string; icon: string }[]; meta: { generatedAt: string; timezone: string; businessName: string } };
